...
export default function MobileMenu({ isOpen, onClose, isAppPage }: MobileMenuProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // Only set animating to false if it was previously true
      if (isAnimating) {
    ...
    <AnimatePresence>
      {(isOpen || isAnimating) && (
        <div className="fixed inset-0 z-50 md:hidden flex flex-col">
          ...
          {/* Menu content */}
          <motion.div 
            className="bg-gray-900 text-white w-full max-w-sm ml-auto h-full flex flex-col shadow-xl relative"
            initial={{ x: "100%" }}
            animate={{ x: 0 }}
            exit={{ x: "100%" }}
          ...
          >
            <div className="flex justify-between items-center p-4 border-b border-gray-800">
              <h3 className="text-xl font-medium text-white">Menu</h3>
              <button
                onClick={onClose}
